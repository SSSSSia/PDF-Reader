...
### 4.1 手工验收清单（T1–T3 批次，2026-09-10 可验）

> 前置：后端（8000）与前端（5173）均已启动；设置页 API 配置可用（SiliconFlow/Qwen3-8B）。
> 整篇导出耗时与 token 成本近似 T0 实测线性放大（RAPTOR 3 页 ≈ 9 分钟/16k token，整篇按页数估算），
> **建议先用页数少/页数中两篇各验一轮**。

| # | 步骤 | 预期 |
|---|------|------|
| 1 | 打开一篇已翻译论文 → 点工具栏「双语PDF」 | 按钮变「双语PDF x%」进度递增，title 显示阶段（解析布局/翻译/生成 PDF）|
| 2 | 导出中再点按钮 | 取消，按钮回「双语PDF」初始态（无残留进程持续跑）|
| 3 | 重新点按同一篇（未取消时） | 后端幂等复用同一任务，不会启动第二个子进程 |
| 4 | 等待完成 | 按钮变「打开双语PDF」；点按 → 系统默认阅读器打开 `*.zh.dual.pdf`（同页并排对照）|
| 5 | 再点一次「双语PDF」 | 缓存命中，瞬时变「打开双语PDF」（不重新翻译、零 token）|
| 6 | 切换左右对照/紧跟/原版模式再回工具栏 | 进度/完成状态不丢（store 跨重挂载保持）|
| 7 | 断网或填错 API 后对新文档点按 | 按钮红色报错，title 携带原因；修正后点按重试成功 |
| 8 | 刷新页面（dev） | 按钮回初始态；再点按 → 后端命中缓存或幂等续跑，不重复翻译 |

已通过的自动化验证（2026-09-10）：tiny PDF 端到端（启动→进度流式→done 100%→产物 1 页
同页英中对照）、缓存命中路径、错误路径、tsc/vite build。第 1/4/5 步的真实论文整篇跑
即 §4 第 1 条的验收主体。
