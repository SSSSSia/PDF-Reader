...
> **状态**：🚧 进行中（T0 决策门 + T1–T3' 完成；T4/T5/T6 待做） ｜ **前置**：阶段 7 进行中（无阻塞依赖，可并行推进；阶段 7 剩余 T5 不阻塞本阶段） ｜ **依据**：用户需求 2026-09-10（达到「沉浸式翻译」排版效果；产品卖点=免费，翻译模型沿用 SiliconFlow 免费 Qwen3-8B）
...
- [ ] **T6 BabelDOC 可选组件化打包（2026-09-11 用户决策）**
  - 决策：主安装包不携带 BabelDOC；用户首次进入「原版PDF·左右对照」且未检测到
    BabelDOC 环境时，弹自绘说明（用途/体积/AGPL）→ 用户点击同意后由应用自动
    下载安装，全程一键、不让用户碰命令行
  - 待设计：分发载体（内嵌 Python embeddable + 离线 wheel 归档 vs 在线安装）、
    安装位置（%APPDATA%/pdf-reader/ 下）、下载进度与失败回退（联动 T4 权重
    下载失败引导）、`_venv_python()` 探测链扩展到该安装位、与 T5 AGPL 声明联动；
    未安装场景复用本阶段确认卡（commit 5dad312）做引导入口
